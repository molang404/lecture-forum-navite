...
import {
    ...
    registerUserSchema,
} from "../../../../schemas/user/registerUserSchema";
...
function AuthRegisterPage() {
    // React에서는 useNavigate() 준비를 해뒀었는데, React-Navigate에서는 useRouter();
    ...
    const onSubmit = async (data: RegisterUserInputType) => {
        try {
            ...
            if (Platform.OS === "web") {
                ...
                // 모바일 환경일 때에는 Alert.alert(제목, 내용, 버튼생성array) 으로 경고창 출력
    ...
    // ios (아이폰)일 때는 padding을 통해 하단에 키보드가 올라갈 공간을 마련해주는 것
    // aos (안드로이드)일 때는 height를 통해 화면이 줄어들도록 적용해주는 것

    // ScrollView 컴포넌트 : 화면이 넘어갈 만큼 넓거나, 길면 스크롤바를 만들어주는 컴포넌트
    // 모바일 앱은 ScrollView로 감싸주지 않으면 스크롤바가 안 나옴. 그래서 감싸줘야 함
    // 심지어 이 ScrollView는 패딩 영역을 주려면 className으로 주지 않고
    // contentContainerClassName 이라는 속성으로 부여해야 함
    // showsVerticalScrollIndicator 속성 : 스크롤바를 보여줄지 여부를 결정
    // keyboardShouldPersistTaps 속성 : 터치 이벤트가 자식 컴포넌트로 전파되는지 여부를 결정하는 속성
    //                                 handled 값을 주지 않으면 키보드가 열려있는 상태에서 터치 시 키보드만 눌림
        ...
        <KeyboardAvoidingView
            ...
            <ScrollView
                ...
                <Card className={twMerge("w-full", "max-w-md", "my-8")}>
                    ...
                    {/*
                        react-hook-form에서 register를 꺼내서 사용하는 방법은 한 번에 처리하는 편의기능
                        Controller 컴포넌트는 react-hook-form에서제공하는 컴포넌트로,
                        폼 필드를 컨트롤할 수 있는 기능을 제공한다.
                        render 속성은 Controller 컴포넌트의 필수 속성으로,
                        폼 필드를 렌더링라는 함수를 지정한다.
                        name 속성은 폼 필드의 이름을 지정한다.
                        control 속성에는 react-hook-form에서 제공하는 control을 넣어줌
                     */}
                    <Controller
                        control={control}
                        name={"username"}
                    ...
                    <Controller
                        ...
                        render={({ field: { onChange, onBlur, value } }) => {
                            return (
                                <InputGroup
                                    label={"닉네임"}
                                    placeholder={"닉네임을 2자에서 10자 이내로 입력해주세요"}
                                    onBlur={onBlur}
                                    onChangeText={onChange}
                                    value={value}
