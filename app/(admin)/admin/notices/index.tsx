...
function AdminNoticeListPage() {
    ...
    const loadNotice = async (targetPage: number, targetSize: number) => {
        try {
    ...
    useEffect(() => {
        loadNotice(currentPage, pageSize).then(() => {});
                ...
                <ScrollView className={"flex-1"}>
                    ...
                    {list.map((item, index) => (
                        <View
                            key={item.id}
                            className={twMerge(
                                ["flex-row", "items-center", "p-4", "transition-all"],
