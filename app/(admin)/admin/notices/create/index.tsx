...
import {
    AdminNoticeInputType,
    adminNoticeSchema,
} from "../../../../../schemas/notice/adminNoticeSchema";
...
function AdminNoticeCreatePage() {
    ...
    const onSubmit = async (data: AdminNoticeInputType) => {
        ...
        // 사용자가 입력한 값과 백엔드애개 던져야 되는 내용이 다르면 그렇게 맞춰서 수정하여
    ...
    return (
        <View className={twMerge("flex-1", "w-full")}>
            ...
            <ScrollView
                ...
                <View className={twMerge("mt-10", ["flex-row", "justify-end", "items-center", "gap-3"])}>
                    <Button variant={"outlined"} color={"secondary"} onPress={() => router.back()}>
                        취소
                    </Button>
                    <Button variant={"contained"} color={"primary"} onPress={handleSubmit(onSubmit)}
                            disabled={isSubmitting}>{isSubmitting ? "저장 중..." : "저장"}</Button>
                </View>
            </ScrollView>
        </View>
    );
